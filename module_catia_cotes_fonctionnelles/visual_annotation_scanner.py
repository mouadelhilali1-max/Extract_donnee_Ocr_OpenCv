...
MAX_TOLERANCE_VALUE = float(os.environ.get("CATIA_MAX_TOLERANCE", "999"))
...
def _white_mask(image: np.ndarray) -> np.ndarray:
    ...
    # Texte/cadres CATIA blancs ou gris clair ; exclut les pièces jaunes et le magenta.
    mask = (b > 145) & (g > 145) & (r > 145) & ((mx - mn) < 60)
    return mask.astype(np.uint8) * 255



def _cyan_frame_mask(image: np.ndarray) -> np.ndarray:
    """Masque des cadres CATIA cyan/bleus, tolérant à l'antialiasing."""
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    hue, sat, val = cv2.split(hsv)
    mask = (hue >= 94) & (hue <= 114) & (sat >= 85) & (val >= 105)
    return mask.astype(np.uint8) * 255
...
def _frame_cell_candidates(image: np.ndarray) -> list[dict[str, Any]]:
    """Cellules/paquets rectangulaires servant d'inventaire physique standard."""
    mask = _white_mask(image)
    h, w = mask.shape
    # La barre CATIA est généralement à gauche. On ne l'OCRise jamais comme annotation.
    mask[:, : max(120, int(w * 0.145))] = 0
    closed = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((2, 2), np.uint8), iterations=1)
    contours, _ = cv2.findContours(closed, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    out: list[dict[str, Any]] = []
    min_short = max(12.0, h * 0.028)
    max_short = h * 0.105
    min_long = max(24.0, h * 0.065)
    max_long = h * 0.22
    for contour in contours:
        area = float(cv2.contourArea(contour))
        if area < 250:
        ...
        if not (min_short <= rh <= max_short and min_long <= rw <= max_long):
            continue
        if not (1.35 <= aspect <= 3.1):
            continue
        # Les contours les plus utiles remplissent une part importante de leur bbox.
...
def _group_frame_cells(candidates: Sequence[dict[str, Any]]) -> list[list[dict[str, Any]]]:
    ...
    while unused:
        ...
        for i in list(unused):
            item = candidates[i]
            if _angle_difference(seed["angle"], item["angle"]) > 5.0:
                continue
            perp = item["cx"] * vx + item["cy"] * vy
            along = (item["cx"] - seed["cx"]) * ux + (item["cy"] - seed["cy"]) * uy
            tol_perp = max(16.0, 0.60 * max(seed["rh"], item["rh"]))
            if abs(perp - base_perp) <= tol_perp and abs(along) <= 330:
                members.append(i)
        if not members:
            members = [seed_i]
        for i in members:
        ...
        # Un groupe standard doit contenir au moins un paquet rectangulaire crédible.
        groups.append(group)
    ...
    for group in sorted(groups, key=lambda g: -sum(x["area"] for x in g)):
        ...
        for previous in final:
            ...
            if math.hypot(cx - pcx, cy - pcy) < 45 and _angle_difference(angle, pa) < 5:
                duplicate = True
                break
        if not duplicate:
            final.append(group)
...
def _tight_group_crop(image: np.ndarray, group: Sequence[dict[str, Any]]) -> tuple[np.ndarray, float, np.ndarray]:
    angle = float(np.median([x["angle"] for x in group]))
    ...
    right_margin = max(45.0, max_rh * 1.6)
    ...
    cx = amid * ux + pmid * vx
    cy = amid * uy + pmid * vy
    crop_width = int(max(280, min(image.shape[1], right - left)))
    crop_height = int(max(92, min(190, max_rh * 3.15)))
    ...
    # Polygone de diagnostic approximatif dans l'image originale.
...
def _series_candidate_scores(texts: Sequence[str], known: set[str]) -> dict[str, float]:
...
def _choose_series(scores: dict[str, float]) -> tuple[str, float, float]:
    ...
    margin = score - second
    if score >= .92 or (score >= .78 and margin >= .08):
...
def _normalise_numeric_text(text: str) -> str:
    ...
    s = re.sub(r"([0-9IL|])\s*\.\s*([0-9A-Z])", lambda m: ("1" if m.group(1) in "IL|" else m.group(1)) + "." + m.group(2), s)
    # Corrections uniquement dans une décimale.
    s = re.sub(r"\.\s*[GS]", ".6", s)
    s = re.sub(r"\.\s*[B]", ".8", s)
    s = re.sub(r"\.\s*[A]", ".4", s)
    return s
...
def _numeric_shape_evidence(cell: np.ndarray) -> tuple[float, int, Optional[int]]:
    ...
    if cell.size == 0:
        return 0.0, 0, None
    mask = cv2.bitwise_or(_white_mask(cell), _cyan_frame_mask(cell))
    binary = (mask > 0).astype(np.uint8)
    h, w = binary.shape
    if h < 5 or w < 4:
        return 0.0, 0, None
    # Les parois du cadre peuvent survivre sur 1 px au bord : elles ne sont pas
    # des glyphes et doivent être retirées avant l'analyse de forme.
    ...
    for i in range(1, n):
        ...
        if area < 2:
            continue
        if hh >= max(7, int(h * .47)) and area >= max(7, int(h * w * .005)):
            if x <= 2 and ww <= 2:  # reliquat de paroi
                continue
            majors.append((x, y, ww, hh, area, ww / max(float(hh), 1.0)))
        elif ww <= max(4, int(w * .15)) and hh <= max(4, int(h * .30)) and area <= max(14, int(h*w*.08)):
...
def _shape_correct_numeric_value(value: float, cell: np.ndarray) -> tuple[float, str]:
    ...
    if leading_one >= .92:
        ...
        if text.startswith("4"):
            ...
            # Une lecture 14 / 14.6 avec un seul glyphe avant le point est une
            # hallucination OCR d'un trait supplémentaire. On conserve la partie
            # décimale réellement lue sans connaître sa valeur à l'avance.
            if digit_count > major_count >= 1 and text.startswith("1"):
                if "." in text:
                    text = "1." + text.split(".", 1)[1]
                else:
                    text = "1"
                changed.append("sur-segmentation-OCR")
    try:
...
def _parse_isolated_numeric_cell(cell: np.ndarray) -> tuple[Optional[float], float, list[str]]:
    """Lit un nombre dans UNE cellule physique isolée, sans symbole ni série.

    Les cadres blancs fins sont le cas le plus difficile : une lecture partielle
    de ``1.6`` donne très souvent ``1``.  On effectue donc plusieurs lectures
    du *même* crop isolé et, lorsqu'une décimale est réellement visible, elle
    prévaut sur sa lecture entière tronquée.  Aucune valeur d'IT n'est imposée.
    """
    if cell.size == 0:
        return None, 0.0, []
    texts: list[str] = []
    votes: dict[float, float] = {}
    # PSM 10 est très utile pour une seule cellule courte, PSM 7/13 couvrent
    # les cas où le point décimal est très proche du bord vertical du cadre.
    # Les deux dernières lectures utilisent un seuillage Otsu local : ce n'est
    # pas lié à une couleur ou à une pièce et améliore les caractères cyan,
    # blancs ou gris à faible contraste.
    variants = (
        (_ocr(cell, 10, whitelist="0123456789.,IL|GSBA", scale=5.2), 1.18),
        (_ocr(cell, 7, whitelist="0123456789.,IL|GSBA", scale=5.0), 1.08),
        (_ocr(cell, 13, white_only=True, whitelist="0123456789.,IL|GSBA", scale=5.4), 1.00),
        (_ocr(cell, 13, whitelist="0123456789.,IL|GSBA", scale=4.2), .82),
        (_local_binary_ocr(cell, psm=10, whitelist="0123456789.,IL|GSBA", scale=5.2), 1.14),
        (_local_binary_ocr(cell, psm=7, whitelist="0123456789.,IL|GSBA", scale=5.0), 1.04),
    )
    for text, weight in variants:
        ...
        # Cellule isolée : on accepte un entier ou une décimale quelconque, sans
        # catalogue propre à une pièce.
        for match in re.finditer(r"(?<![0-9])([0-9]{1,4}(?:\.[0-9]{1,3})?)(?![0-9])", normalized):
            ...
            if 0 < value <= MAX_TOLERANCE_VALUE:
                # Une décimale contient une information supplémentaire qui est
                # absente dans la lecture tronquée de ce même nombre.
                bonus = .34 if "." in match.group(1) else 0.0
                votes[value] = votes.get(value, 0.0) + weight + bonus
    if not votes:
        return None, 0.0, texts

    # Le vote reste volontairement générique : tous les réels dans la plage
    # définie par CATIA_MAX_TOLERANCE sont possibles. Une décimale est favorisée
    # uniquement face à sa lecture entière tronquée, jamais selon son nombre de
    # chiffres après la virgule ni selon une liste de valeurs propre à une pièce.
    decimals = {value: score for value, score in votes.items() if value % 1}
    integers = {value: score for value, score in votes.items() if not value % 1}
    if decimals:
        decimal_winner = max(decimals, key=lambda value: decimals[value])
        # ``1`` peut être une lecture partielle de ``1.6``. Une décimale vue
        # au moins une fois devient donc prioritaire sur cet entier précis.
        truncated_score = integers.get(float(int(decimal_winner)), 0.0)
        if decimals[decimal_winner] >= .90 or decimals[decimal_winner] + .55 >= truncated_score:
    ...
    # Tesseract confond parfois le « 1 » étroit de CATIA avec « 4 ». Cette
    # correction ne concerne qu'une cellule dont l'OCR a produit exactement 4
    # et dont OpenCV prouve un unique glyphe central étroit et vertical. Un vrai
    # 4, plus large, n'est donc pas modifié. C'est une règle de forme générale,
    # indépendante de la valeur attendue ou de la pièce.
    one_hint = _single_digit_one_shape_hint(cell)
    if abs(float(winner) - 4.0) < 1e-9 and one_hint >= .94:
        winner = 1.0
        confidence = max(confidence, one_hint)
        texts.append("OpenCV-glyph: 1")

    # V9 : vérification de forme multi-glyphes. Elle récupère notamment un point
    # décimal réellement visible mais perdu par OCR, et les 1 lus 4/14.
    corrected, correction = _shape_correct_numeric_value(float(winner), cell)
    if correction and abs(corrected - float(winner)) > 1e-9:
        winner = corrected
        confidence = max(confidence, .965)
...
def _lsd_wall_sets(image: np.ndarray, min_boundaries: int = 3) -> list[dict[str, Any]]:
    """Détecte les vraies parois verticales d'un cadre redressé avec LSD.

    Contrairement à une projection binaire, LSD fournit la longueur de chaque
    segment. Les traits des chiffres (ex. le «1» de 1.6) sont donc plus courts
    que les parois complètes et ne sont plus pris pour des séparateurs de cellule.
    """
    ...
    for x1, y1, x2, y2 in detected.reshape(-1, 4):
        ...
        if length < max(6.0, h * .07) or length > max(60.0, h * .78):
            continue
        angle = abs(math.degrees(math.atan2(dy, dx)))
        if abs(90.0 - angle) > 8.0:
            continue
        raw.append({
            "x": float((x1+x2)/2.0), "ym": float((y1+y2)/2.0),
            "length": length, "ylo": float(min(y1,y2)), "yhi": float(max(y1,y2)),
        })
    if not raw:
        return []

    # Fusion des deux arêtes antialiasées d'une même paroi.
    clusters: list[list[dict[str,float]]] = []
    for item in sorted(raw, key=lambda q:q["x"]):
        target = None
        for cluster in clusters:
            mx = float(np.mean([q["x"] for q in cluster]))
            my = float(np.mean([q["ym"] for q in cluster]))
            if abs(item["x"]-mx) <= 3.3 and abs(item["ym"]-my) <= 5.0:
                target = cluster; break
        if target is None:
            clusters.append([item])
        else:
            target.append(item)
    reps = [max(cluster, key=lambda q:q["length"]) for cluster in clusters]

    wall_sets: list[dict[str,Any]] = []
    for seed in reps:
        aligned = [q for q in reps if .70 <= q["length"]/seed["length"] <= 1.42
                   and abs(q["ym"]-seed["ym"]) <= max(3.5, .22*seed["length"])]
        aligned.sort(key=lambda q:q["x"])
        selected: list[dict[str,float]] = []
        for item in aligned:
            if not selected or item["x"]-selected[-1]["x"] > 5.0:
                selected.append(item)
            elif item["length"] > selected[-1]["length"]:
                selected[-1] = item
        if len(selected) < min_boundaries:
            continue
        xs = [q["x"] for q in selected]
        y1 = float(np.median([q["ylo"] for q in selected]))
        y2 = float(np.median([q["yhi"] for q in selected]))
        frame_h = y2-y1
        if frame_h < 8:
            continue
        gaps = np.diff(xs) if len(xs) > 1 else np.array([])
        plausible = sum(.42*frame_h <= float(g) <= 4.8*frame_h for g in gaps)
        score = float(np.median([q["length"] for q in selected])) + 1.55*min(len(xs),7) + plausible
        wall_sets.append({"bounds":xs,"y1":y1,"y2":y2,"height":frame_h,"score":score})

    unique: list[dict[str,Any]] = []
    for item in sorted(wall_sets, key=lambda q:q["score"], reverse=True):
        xs=item["bounds"]
        duplicate=False
        for prev in unique:
            if len(xs)==len(prev["bounds"]) and float(np.mean(np.abs(np.array(xs)-np.array(prev["bounds"])))) < 3.0:
...
def _best_lsd_frame_geometry(crop: np.ndarray, min_boundaries: int = 3) -> dict[str, Any]:
...
def _lsd_group_it(crop: np.ndarray) -> tuple[Optional[float], float, str, dict[str,Any]]:
    """Lit la 2e cellule en partant des parois complètes, puis prouve A."""
    geom=_best_lsd_frame_geometry(crop, min_boundaries=4)
    if not geom or len(geom["bounds"]) < 4:
        return None,0.0,"LSD: moins de 4 parois",geom
    bounds=geom["bounds"]; y1=geom["y1"]; y2=geom["y2"]; fh=geom["height"]
    # Pour un crop construit autour du groupe, le premier ensemble de parois de
    # pleine hauteur correspond au début du cadre. On teste néanmoins les deux
    # premiers départs et on exige la référence A.
    candidates=[]
    for i in range(min(2,len(bounds)-3)):
        x0,x1,x2,x3=bounds[i:i+4]
        gaps=(x1-x0,x2-x1,x3-x2)
        if not (.45*fh <= gaps[0] <= 3.0*fh and .45*fh <= gaps[1] <= 4.6*fh and .40*fh <= gaps[2] <= 4.6*fh):
            continue
        ...
        a_cell=crop[max(0,int(y1)+py):min(crop.shape[0],int(y2)-py), max(0,int(x2)+1):min(crop.shape[1],int(x3)-1)]
        proven,a_texts=_datum_a_in_next_cell(a_cell)
        if not proven: continue
        score=conf + .08 - .04*i
        candidates.append((score,value,conf,texts,a_texts,i))
    if not candidates:
        return None,0.0,"LSD: aucune 2e cellule suivie de A",geom
    _score,value,conf,texts,a_texts,i=max(candidates,key=lambda q:q[0])
    return value,min(.995,max(.94,conf)),f"LSD parois physiques depart={i}; OCR={texts}; next-A={a_texts}",geom
...
def _single_digit_one_shape_hint(cell: np.ndarray) -> float:
    """Retourne une confiance [0..1] que la cellule contient le chiffre 1.

    CATIA dessine parfois le chiffre 1 d'une façon que Tesseract confond avec 4
    lorsque la cellule est très petite. On n'utilise ce test que pour départager
    1 et 4 : un '1' est un composant central étroit et haut, alors qu'un vrai '4'
    est sensiblement plus large.
    """
    if cell.size == 0:
        return 0.0
    # Même règle pour cadres blancs, gris ou cyan : le masque réunit les traits
    # d'annotation sans dépendre d'une pièce ni d'une couleur particulière.
    mask = _annotation_stroke_mask(cell)
    h, w = mask.shape
    binary = (mask > 0).astype(np.uint8)
    ...
    for i in range(1, n):
        ...
        if touches:
            continue
        if area < max(7, int(h * w * .008)):
            continue
        if hh < max(8, int(h * .30)):
            continue
        cx = x + ww / 2.0
        cy = y + hh / 2.0
        if not (w * .18 <= cx <= w * .82 and h * .12 <= cy <= h * .88):
            continue
    ...
    if not candidates:
        return 0.0
    area, ww, hh, aspect = max(candidates)
    if aspect <= .46:
        return .96
    if aspect <= .52:
        return .88
...
def _frame_group_it(
    ...
    # Correction ciblée 4 -> 1 uniquement lorsque la morphologie du glyphe
    # prouve un caractère étroit/vertical. Un vrai 4 n'est donc pas modifié.
    if one_hint >= .88 and (value is None or abs(float(value) - 4.0) < 1e-9):
        value = 1.0
        conf = max(conf, one_hint)

    if value is None:
        return None, 0.0, f"cellule groupe x={x1}:{x2}; OCR={texts}; hint1={one_hint:.2f}"
    return (
        value,
        min(.995, max(conf, .78)),
        f"cellule IT reconstruite par groupe x={x1}:{x2}; OCR={texts}; hint1={one_hint:.2f}",
    )
...
def _rescue_missing_series(
    ...
    for image_path in images:
        ...
        for group in groups:
            if not remaining:
                break
            crop, angle, polygon = _tight_group_crop(image, group)
            ...
            # Même règle que le passage principal : cellule IT prouvée avant
            # toute reconstruction approximative à partir du groupe.
...
def _draw_debug(image: np.ndarray, observations: Sequence[PhysicalObservation], output: Path) -> None:
    canvas = image.copy()
    for obs in observations:
        poly = obs.crop_polygon.reshape((-1, 1, 2)).astype(np.int32)
        cv2.polylines(canvas, [poly], True, (0, 255, 0), 2)
        x, y = int(obs.crop_polygon[:,0].min()), int(obs.crop_polygon[:,1].min())
        label = f"{obs.series_code} IT={obs.tolerance_value:g}" if obs.tolerance_value is not None else obs.series_code
        cv2.putText(canvas, label, (max(5, x), max(18, y-5)), cv2.FONT_HERSHEY_SIMPLEX, .55, (0,255,0), 2, cv2.LINE_AA)
...
def _consensus(observations: Sequence[PhysicalObservation], known: set[str]) -> list[VisualAnnotation]:
    ...
    for code in sorted(by_series):
        ...
        # Vote IT pondéré par confiance. Une décimale cohérente reçoit un bonus si 1 et 1.x coexistent.
        weights: dict[float, float] = {}
        for item in items:
            value = float(item.tolerance_value)
            weights[value] = weights.get(value, 0.0) + max(.2, item.confidence)
        for value in list(weights):
            if value % 1:
                integer = float(int(value))
                if integer in weights:
                    weights[value] += .35 * weights[integer]
        winning_it = max(weights, key=lambda v: weights[v])
...
def _reconcile_ambiguous(
    ...
    for score, idx, code in sorted(choices, reverse=True):
        ...
        ranking = sorted(scores.items(), key=lambda x: x[1], reverse=True)
        second = ranking[1][1] if len(ranking) > 1 else 0.0
        # Ne réconcilie que si la preuve locale reste raisonnable.
        if score < .68 and score - second < -.10:
            continue
...
def scan_annotation_captures(
    *,
    ...
    cyan_style_seen = False
    legacy_style_seen = False
    legacy_image_paths: list[Path] = []
    ...
    for image_index, image_path in enumerate(images, start=1):
        ...
        # Les cadres cyan droits gardent leur lecture directe par cellules. Les
        # autres styles passent d'abord par le groupement minAreaRect de V3.2
        # (très robuste pour les cadres blancs inclinés), puis le moteur orienté
        # V8 sert uniquement de complément si aucun groupe physique n'est lu.
        cyan_candidates = _cyan_cell_candidates(image)
        cyan_groups = _group_cyan_cells(cyan_candidates)
        credible_cyan = [g for g in cyan_groups if len(g) >= 3]

        image_observations: list[PhysicalObservation] = []
        condition_hint = False
        groups: list[list[dict[str, Any]]] = []
        style = "CYAN" if credible_cyan else "BLANC"

        if credible_cyan:
            cyan_style_seen = True
            groups = credible_cyan
            for group_index, group in enumerate(groups, start=1):
                obs = _cyan_group_observation(image_path, image, group, known)
                if obs is not None:
            ...
            legacy_style_seen = True
            legacy_image_paths.append(image_path.resolve())
            candidates = _frame_cell_candidates(image)
            groups = _group_frame_cells(candidates)
            min_area = max(260.0, image.shape[0] * image.shape[0] * .0018)
            groups = [g for g in groups if sum(item["area"] for item in g) >= min_area]
            style = "BLANC_INCLINE" if groups else "PHYSIQUE_ORIENTE"
            for group_index, group in enumerate(groups, start=1):
                crop, angle, polygon = _tight_group_crop(image, group)
                obs, hint = _standard_observation(
                    image_path, crop, angle, polygon, known, group=group,
                )
            ...
            # Une image sans groupe V3.2 peut encore contenir un cadre très fin,
            # discontinu ou d'une autre couleur. Le moteur orienté est alors le
            # filet de sécurité, sans remplacer les groupes déjà fiables.
            if not groups:
        ...
        image_observations = list(local_best.values())
    ...
    # Rescue V3.3 : pour un style cyan, on NE relance pas les rotations/Tesseract
    # globaux. On repasse seulement les cadres cyan déjà inventoriés en limitant
    # le dictionnaire aux séries manquantes. C'est rapide et réduit l'ambiguïté.
    if missing and cyan_style_seen:
        print("      Rescue local cyan : " + ", ".join(missing), flush=True)
        targets = set(missing)
        rescued: list[PhysicalObservation] = []
        for image_path in images:
            image = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
            if image is None:
                continue
            groups = [g for g in _group_cyan_cells(_cyan_cell_candidates(image)) if len(g) >= 3]
            for group in groups:
        ...
        if rescued:
            ...
            print("      Rescue cyan obtenu : " + ", ".join(f"{x.series_code}={x.tolerance_value:g}" for x in rescued), flush=True)
            rows = [asdict(item) for item in _consensus(observations, known)]
            found = {row["series_code"] for row in rows}
            missing = sorted(known - found)

    # Rescue série-guidée : ne cherche que les codes encore manquants de l'arbre.
    if missing and legacy_style_seen:
        print("      Rescue V9.0 série-guidée locale : " + ", ".join(missing), flush=True)
        rescued = _rescue_series_guided(legacy_image_paths, known, set(missing), diagnostics)
        if rescued:
            observations.extend(rescued)
            rows = [asdict(item) for item in _consensus(observations, known)]
            found = {row["series_code"] for row in rows}
    ...
    # Rescue hybride : V3.2 repart des groupes inclinés en limitant le
    # dictionnaire aux seules séries manquantes. Il est particulièrement utile
    # pour un cadre partiellement masqué dans une vue très chargée.
    if missing and legacy_style_seen:
        print("      Rescue V9.0 groupes inclinés : " + ", ".join(missing), flush=True)
        rescued = _rescue_missing_series(images, known, set(missing))
        if rescued:
            observations.extend(rescued)
            print(
                "      Rescue groupes obtenu : "
                + ", ".join(f"{item.series_code}={item.tolerance_value:g}" for item in rescued),
                flush=True,
            )
            rows = [asdict(item) for item in _consensus(observations, known)]
            found = {row["series_code"] for row in rows}
    ...
    # Dernier filet de sécurité : détecteur orienté plus permissif pour traits
    # fins, couleurs atypiques et cadres dont les contours sont discontinus.
    if missing and legacy_style_seen:
        print("      Rescue V9.0 cadres OpenCV orientés : " + ", ".join(missing), flush=True)
        rescued: list[PhysicalObservation] = []
        for image_path in legacy_image_paths:
            image = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
            if image is None:
                continue
